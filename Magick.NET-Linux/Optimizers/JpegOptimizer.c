...
#include "ImageMagick/include/jpeg/jpeglib.h"
#include "ImageMagick/include/jpeg/jerror.h"
#include <setjmp.h>
...
static void InitializeSource(j_decompress_ptr decompress_info)
{
  ...
  source->inputFile = fopen(client_data->inputFileName, "rb");
...
static size_t DetermineQuality(j_decompress_ptr decompress_info)
{
  ...
  */
  ...
  return (int)fmin((float)quality, 85.0f);
}
...
static void InitializeDestination(j_compress_ptr compress_info)
{
  ...
  destination->outputFile = fopen(client_data->outputFileName, "wb");
